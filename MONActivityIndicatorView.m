...
- (UIView *)createCircleWithRadius:(CGFloat)radius
                             color:(UIColor *)color
                         positionX:(CGFloat)x {
    CGFloat top = (viewHeight - [self intrinsicContentSize].height)/2;

    UIView *circle = [[UIView alloc] initWithFrame:CGRectMake(x, top - radius/2, radius * 2, radius * 2)];
...
- (void)addCircles {
    CGFloat left = (viewWidth - [self intrinsicContentSize].width)/2;
    for (NSUInteger i = 0; i < self.numberOfCircles; i++) {
        ...
        UIView *circle = [self createCircleWithRadius:self.radius
                                            ...
                                            positionX:left+(i * ((2 * self.radius) + self.internalSpacing))];
...
//        [[[obj view] layer] removeAnimationForKey:@"scale"];
